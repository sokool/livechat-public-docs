...
This documentation describes the **Agent Chat Real-Time Messaging API v3.2**. This is the **developer preview** version recommended only for the **experimental** use. [Read more about versionig...](/messaging/#api-versioning)
...
|                 |                                                                                                                                                                                                                                                                                                                                                         |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Chats**       | [`list_chats`](#list-chats) [`get_chat_threads_summary`](#get-chat-threads-summary) [`get_chat_threads`](#get-chat-threads) [`list_archives`](#list-archives) [`start_chat`](#start-chat) [`activate_chat`](#activate-chat) [`deactivate_chat`](#deactivate-chat) [`follow_chat`](#follow-chat) [`unfollow_chat`](#unfollow-chat)                 |
...
| `limit`            | No       | `number`  | Default: 10, maximum: 100                                                                  |
| `min_events_count` | No       | `number`  |                                                                                            |

`limit` and `min_events_count` can't be both used at the same time.

When `min_events_count` is used, reponse will contain as many latest threads that their combined events count will be at least equal to given value.
